...
**Two more owner reports from the same session are open and need the owner's input, not more code changes yet:**

- **The home yard/room cat appears to jump to the middle of the screen on entry.** Investigated with a deterministic clock-driven harness (not live browser timing, which this environment cannot reliably simulate): re-entering the *same* scene (yard after leaving to the map and coming straight back) correctly preserves the cat's last position - confirmed, not a bug. Switching *between* the yard and the room, though, always resets the cat to sit at that scene's door (dead centre, `x:50%`) - this is deliberate (`home-pet.js`'s `crossDoor`, "the cat walks in with you"), not a malfunction, but if the owner is doing this often while decorating, it would read exactly as "always transported to the middle." Needs the owner to confirm whether this is what they are seeing, since the fix (if any) is a design choice - stop resetting on a room switch, or something else - not a bug fix.
- **"No click to go out of the yard."** Not reproduced: `.home-scene-back` (`← Lantern Alley` in the yard, `← 庭` in the room) exists, is visible, and is clickable at every viewport tested (1280x800, 1000x700, 375x812 mobile). Needs a screenshot or more specific repro steps from the owner before a code change is justified.
