...
Last updated: 2026-09-26 (v452, shift board with portraits on feature branch)
...
- **Portraits:** done in v452. The owner's twelve portraits are used on the
  board, the helped-guest bar and the ending.
...
passed, but rendered motion and edge placement were not visually checked
because browser automation stopped. Ask the user to test this build on mobile
before calling the pet issue fixed. Existing missing Inn audio remains out of
scope.

**v448 (2026-09-25):** a Shiba can keep walking when the room contains a
center piece and both folding screens. That realistic layout blocked all five
old dog routes and made `nextAnchor()` return null forever. Two front-tatami
anchors provide a clear grounded lane, protected by a regression test using
the measured kotatsu and screen footprints.
...
440 x 440 subjects on a shared ground line. Each dog receives a stable profile
from its instance ID: gait, phase, pace, route direction and rest timing differ,
and resting dogs avoid copying a sibling's current idle pose when another pose
is available. Focused home/pet tests cover the two gait assets and multi-dog
divergence.
...
**Latest v386 status (2026-09-16):** re-audited all indoor object dimensions,
artwork bounds, room depth scaling, surface clearance, and scene lighting. No
new size mismatch was found after the Kotatsu correction. Free home placement
targets are now 52px instead of 44px; object landing coordinates are unchanged.
The targeted decor, home, yard, and touch checks pass. sw.js is v386.

**Latest v387 status (2026-09-16):** the room now has 16 base placement
locations instead of 11: six floor, four wall, four post, one eave, and one
window-sill location. New positions are constrained to visible room surfaces
...
**Latest v398 status (2026-09-17):** the v397 屏風 rotation approach was
incorrect because it tilted the upright posts. The two screen slots now use
...
an iron link chain, and the hearth at the existing centre-back floor slot.
The separate v390 SVG support was removed because it did not share the asset's
perspective. The item retains its floor contact shadow, scene-aware lighting,
and model-level placement restrictions. The targeted decor, PWA, and syntax
checks need to be run before a release. sw.js is v391.

**Latest v390 status (2026-09-16):** the room now has 18 base placement
...
**The branch beneath v324 includes** the kimono characters, gravel-integrated plant art, guarded home-scene cleanup, a non-overlapping Entrance action dock, corrected first-home starter stock, the completed Inn learning redesign, a fix for a learner-reported crash on returning to the Entrance or the Inn after a home visit, a fix for content-heavy Inn rooms scrolling before any answer was given, a fix for the home's cat resetting to the door on every yard/room switch, scene-painted exits for both the yard and the room, matching the existing house hotspot, a `?skip=1` testing flag past character selection and the Entrance, per-question/whole-stage skip controls inside the Inn stage itself, the same per-question skip extended to Episode 1, a UI audit pass over the opening, the Inn and the home, the story path feeding the delayed-review schedule, and a cold open that puts a guest in front of the learner before the Inn's three days. Inn screens now label the reading path from Kon's request to the answer workspace, and every episode answer names the exact catalog word, reading and sense after the attempt. Context-sensitive Inn furigana no longer applies standalone dictionary readings to conjugated forms. Day 3 tests 温める and 引き受ける with recorded four-choice listening instead of weak room actions, and a reload preserves first-attempt misses, the day's mistake list, and the current review-ladder rung. The audio-only 調整 task has an explicit replay button and clear Earlier/Later controls, and its scenario is real dinner seating rather than an unexplained arrival constraint. Desktop Inn requests and answer areas now share a top baseline across all 55 question states, including episode questions and short reply lists. Retryable Inn misses now describe the result as `不正解` and explicitly offer retry or continuation instead of ordering a retry beside a next-task button. On phones, the Inn keeps its stacked layout, while the yard and interior keep their original scene coordinates inside a 300px to 390px-tall pannable camera, remember separate pan positions, and retain a fixed exit. `sw.js` agrees with all versioned local assets in `index.html`. Every spoken line in the game has a pre-rendered audio clip (see item 2 in section 11A).
...
**The 調整 (Day 1) question's premise did not survive scrutiny either, three times over.** Reported live: real inns do not let staff adjust a guest's checkout time - it went through a full rewrite from "staff picks the guest's checkout" to "staff schedules when cleaning starts, given the guest's own stated checkout," dropped an irrelevant train/travel-time calculation once that was flagged too, and added a stated one-hour gap between checkout and cleaning once an instant handoff was flagged a third time. A later review found the former Day 2/Day 3 arrival-and-lobby premise was also too vague to justify its timing gap. Those tasks now use dinner seating instead: two groups request the same time, one group is served at a time, and the learner schedules the start times around the two-hour meal. See the 2026-09-04 and 2026-09-07 change log entries.
...
**Inn learning redesign is implemented and played clean start to finish.** The current route is 5 Learn, 5 Practice and 5 Challenge tasks; first-attempt-only evidence; a three-rung review ladder for misses; selective reading help; and a five-beat shift tracker for each episode. The stage opens with its five-word job board, then an unscored cold open, then the three days. Learn cards show only the target word, reading and concise meaning. Existing Inn scene art was sufficient, so no new raster art was added. `INN-REBUILD-PLAN.md` records the current design and the remaining content work.
...
The v276 browser walkthrough played the board-first opening, cold open, all five items on Day 1 and Day 2, and all five audio-only Challenge items into Episode 1. 温める and 引き受ける rendered as replayable four-choice listening; Episode 1 listed all ten of its actual targets, hid the target before answering, and showed it after both an incorrect and a correct answer. Separate home captures at 375x812 and 844x794 verified the mobile camera and unchanged desktop scene. The full automated suite passes **462/462**.
...
Fresh home visits now start with an empty yard and empty storage. The home directs the learner to Moonview Inn, where the cushion and camellia seed are earned as separate rewards rather than given at once. The placement tutorial begins only after both items are actually owned. Saves containing the retired automatic `starter-maple` remove that special id during normalization; purchased maples are preserved.
...
**The house and garden.** `わが家` sits in the middle of the map and is never locked - coins may decide what goes inside it, but nothing about understanding decides whether a learner can go home. It opens on an illustrated yard; the house leads to an interior. The first Inn reward earns a cushion and Episode 1 earns a camellia seed. Once both are owned, the home tutorial teaches planting, placement and moving through actions rather than a Next button; `使いかた` replays the explanation without handing anything out twice.
...
A `?bust=anything` on the page URL only reloads `index.html` itself; the scripts underneath still come from cache with their old `?v=`. Bump the version. The reverse also bites in an agent's browser pane: bumping `?v=` while the pane still holds the *old* `index.html` loads the old scripts anyway, because the document itself came from cache. Bump the version **and** put a throwaway param on the page URL.

**A layout check in an agent browser pane is worthless until the viewport is real.** The pane reports `document.documentElement.clientWidth === 0` and `matchMedia('(min-width:761px)').matches === false` by default, so every rule inside this project's `@media(min-width:761px)` and `@media(max-width:1400px)` blocks - which is where most of the Inn's desktop layout lives - is simply inactive, and the pane measures a layout no human sees. Screenshots look plausible throughout. Three consecutive "fixed and verified" reports on the same layout bug shipped nothing because of this. Call `resize_window` with an explicit `width`/`height` (not `preset:"desktop"`, which clears emulation back to the broken state), then assert `matchMedia(...).matches` is true before believing any measurement. See the 2026-09-04 "actually composed" change log entry.
...
**The Moonview Inn's introductory route is the one named exception to this 3-3-4 shape**, and is a separate stage from its own four 3-3-4 episodes. `N2HomeInnStage` teaches the Inn's first five words - 揃える, 取り替える, 温める, 調整, 引き受ける - as **5 Learn, 5 Practice and 5 Challenge**. All five words therefore reach the final audio-only gate. Day 3 keeps room actions for 揃える, 取り替える and 調整, where performing the task demonstrates the distinction; 温める and 引き受ける use recorded four-choice listening, avoiding a meaningless object move and a 50/50 accept/decline guess. Only first-attempt correct answers count as training evidence. Missed Challenge words enter a three-rung review ladder, and reloads preserve the exact rung and mistake evidence. Clearing the shift is silver; delayed retention is gold. See `INN-REBUILD-PLAN.md` and the 2026-09-07 change log entries.
...
2. **Native review of the Japanese.** 200 questions, five story arcs, all of Kon's dialogue, the eight tutorial lines at the house, every garden and shop string - authored in this project, never checked by a native speaker. Everything else is cheap to change; this is the one thing nobody else can do. `?review=1` walks all 215 items in place with the clock off and an おかしい checkbox. `generate-audio.py` hashes its input, so corrections later cost only the lines that changed.
   ...
   **Lines written by Claude and not yet reviewed.** `?review=1` walks the 215 question items; these are dialogue and chrome, so they sit outside it. The owner reviews the Japanese once stage creation is finished, so this list is kept current rather than acted on.
   ...
   | 夕食の時間を決める / Aグループ夕食 / Bグループ夕食 / 二つのグループの夕食開始時刻を（　　）ください。 | `n2-home-inn-stage.js`, Day 2 and Day 3 調整 labels and cloze (rewritten 2026-09-07) |
   | コン：「夕食の時間に、ＡグループとＢグループから同じ時刻の希望をいただきました。食事処は一組ずつご案内します。」 | `n2-home-inn-stage.js`, Day 3 調整 narration (rewritten 2026-09-07) |
   | Aグループは18時以降、Bグループは20時までに夕食を始められます。一組の食事には2時間かかります。夕食の開始時刻を調整してください。 | `n2-home-inn-stage.js`, Day 3 調整 request (rewritten 2026-09-07) |
   | Aグループは18時、Bグループは20時になりました。これで順番に夕食をお出しできます。 | `n2-home-inn-stage.js`, Day 3 調整 success reply (rewritten 2026-09-07) |
...
2. ~~**Approve the audio run.**~~ **Done 2026-09-04.** Ran across two passes that session: the first (510 lines) cleared the whole prior backlog; the second (3 lines) picked up the 調整 encounter's final rewording after it changed again post-generation. A later user-approved, scoped three-clip run on 2026-09-07 replaced the former Day 3 arrival scenario with dinner seating. Every spoken line in the game now has a clip; `node --test` confirms it. Any future content edit still needs its own run - `generate-audio.py` sends Japanese text to Microsoft Edge TTS, an external service, so it stays approval-gated every time, not just until the backlog clears.
