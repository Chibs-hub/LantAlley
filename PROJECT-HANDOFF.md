...
**The current working candidate is v226 on `codex/inn-learning-redesign` and is not committed or pushed.** The production baseline remains v218 until the owner authorizes that. The candidate includes the kimono characters, gravel-integrated plant art, guarded home-scene cleanup, a non-overlapping Entrance action dock, corrected first-home starter stock, the completed and fully verified Inn learning redesign, a fix for a learner-reported crash on returning to the Entrance or the Inn after a home visit, a fix for content-heavy Inn rooms scrolling before any answer was given, and a fix for the home's cat resetting to the door on every yard/room switch. `sw.js` agrees with all 26 `index.html` stamps. `node --test` passes **406/406**.
...
**One more owner report from the same session is open and needs the owner's input, not more code changes yet:**

- **"No click to go out of the yard."** Not reproduced: `.home-scene-back` (`← Lantern Alley` in the yard, `← 庭` in the room) exists, is visible, and is clickable at every viewport tested (1280x800, 1000x700, 375x812 mobile). The owner confirmed they simply didn't notice the text-link button, rather than it being missing or broken - this is a discoverability question (would a more visible affordance, e.g. a clickable spot in the scene itself like the door hotspot, be worth adding?) rather than a bug. No code change made; needs the owner to say whether they want that enhancement.
