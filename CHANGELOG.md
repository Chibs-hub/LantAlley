...
### 2026-09-17 - Make the next lesson obvious on the map (v403)
...
**Repairing a reading nearly renamed another word's id.** Ids are derived from readings, two words can want the same one, and the loser takes a "-2" suffix - so a row that gains a reading can take an id another word already answers to. 灰 and はい are both read はい; 灰 is what a shrine question teaches and has always held `w-hai`. Named in sorted order, the repaired はい took it, and `shrine-e04-q04` became a question about "yes" instead of about ash. The suite caught it, which is the only reason this paragraph is not a bug report.

Repaired rows are therefore named last. They cannot take an id another word already answers to, and every id that existed before the repair is unchanged after it. Nineteen ids move, all of them names the repaired rows never really had - `w-x`, `w-x-2`, `w-kapeto` for じゅうたん, `w-i-2` for 賛成 - plus 割る, which gets `v-waru` back now that しまう is not squatting on it with a reading that romanised from "（終わる）". No authored question names any of them. A learner's saved schedule entries for those nineteen words go stale and the words return as if new; nothing else in a save is keyed by id.
...
Two of the supplied files carried transparent space beneath the object - the teapot 125px of 1254, a tenth of its height - which is the one thing that cannot be allowed, because `anchorY:100` means an object's base is the bottom row of its picture and padding lifts it off whatever it stands on. All six were cropped to their ink before conversion, then sized to about four times the width they are ever drawn at rather than a flat cap: a teapot renders 17px across, so 640px of it was thirty-five times more than the shell needed to carry. 219KB for the six.

Their `PRESENTATION` rows moved with them. `anchorY` went back to 100 for the four that stand on something - the vector anchors of 63.5, 69.2 and 65.4 described where a drawing stopped inside a box it did not fill - and every `width` came down to the object rather than the box: 急須 8 to 4.6, 本 9 to 5.6, 火鉢 14 to 11.5, 小さな鉢 14 to 7.6, 面 9 to 3.8. 扇 went up to 11.5, an open sensu being wider than the drawing suggested.

The floating that prompted this was not any of them. The shelf's own picture had 4.8% of transparent height under it, so both shelves stood off the floor and carried everything on them up as well. Cropping it changed its aspect from 1.385 to 1.405 and moved every plank, so the slot heights were re-derived by scanning the asset for rows that are more than 55% opaque and taking the top of each run: 0.0147, 0.3096, 0.5381, 0.7445 of the height, which at a 15% shelf standing at y=73 puts the top plank at 54.30 and the bottom at 68.15. Measured back afterwards, all four objects land on their planks exactly.

面 also moved from `wall` to `post`. Three things wanted two stretches of flat wall while a pillar stood empty, and a mask is hung on a pillar in a room like this anyway - so every hanging piece in the catalogue now has somewhere to go.

Shelf space cannot grow, and the reason is worth recording so it is not retried. At the smallest scene the room is drawn at, 533x300, a shelf is 80x58px; the widest vertical gap between any two of its planks is 38px, under the 44px touch minimum, so any two slots must clear 44px horizontally. A third needs 44px from both - 88px of span, and the shelf has 80. Two per shelf is the ceiling, four in the room, against seven `shelf` pieces in the catalogue. More would need a third piece of furniture, and the back wall is full between its two fusuma and the doorway while both side walls are drawn receding, so the frontal cabinet asset would sit at the wrong angle. Cache is v334; `node --test` passes (527).
