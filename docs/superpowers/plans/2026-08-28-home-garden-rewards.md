...
- Create: `assets/home/decor/floor-cushion-navy-v1.png`
- Modify: `home-room.js`
...
Use the supplied image inventory first. The supplied tatami room becomes the interior background and the supplied navy cushions become the starter cushion after cleanup and optimization. Use built-in image generation with the approved interior, Entrance, and Inn assets as style references for the missing wide yard/exterior background and four transparent camellia growth stages. Copy final outputs into the paths above. Inspect every file for matching perspective, clean alpha, no text, no watermark, and no cropped silhouette.
...
The free flower is a camellia instance and the free decor is `floor-cushion-navy`. Claims check both flags and current ownership/instances before adding anything. Replay mode shows the same explanation without executing claim operations.
